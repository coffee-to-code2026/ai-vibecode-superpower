...
- 先读取目标项目根 `AGENTS.md`，并确认项目允许写入 `opencode.json`（或 `.codex/`，取决于 `--config`）、`.gitignore` 和 `AGENTS.md`。
...
1. 运行 `configure --config opencode`：仅在不存在冲突时写入项目 `opencode.json` 的 `mcp.codegraph`、`/.codegraph/` 忽略规则及项目根 `AGENTS.md` 的唯一 `## CodeGraph 与 RTK` 受管标题。该标题内同时包含运行时工具规则和 `agent-toolchain` 管理路由；旧版 `## AI 工具` 标题不会被覆盖。
...
从当前已加载 skill 的绝对 `SKILL.md` 路径定位驱动：把末尾 `SKILL.md` 替换为 `scripts/agent-toolchain.ps1`（Windows）或 `scripts/agent-toolchain.sh`（macOS/Linux）。该绝对 skill 路径由 opencode 在本会话的 skill base directory 中提供。不得扫描 `plugins/cache`、按时间挑选版本、使用开发仓库路径，或自行展开 home 环境变量。
...
- CodeGraph MCP 配置生效通常需要新建 opencode 会话或重启客户端。
