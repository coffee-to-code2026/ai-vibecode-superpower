...
usage() {
  cat <<'EOF'
Usage:
  agent-toolchain.sh configure --project PATH --config opencode
  ...
  --config 指定 MCP 配置写入目标：本适配包仅支持 opencode（写入项目 opencode.json 的 mcp 字段）。Codex/ZCode 项目请使用共享驱动的对应落点。
EOF
}
...
parse_args() {
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --config) [ "$#" -ge 2 ] || die "--config 缺少值"; CONFIG_TARGET=$2; shift 2 ;;
      --apply) APPLY=1; shift ;;
...
configure_project() {
  case "$CONFIG_TARGET" in
    codex) die 'opencode 适配包不再支持 --config codex 落点；请使用共享的 agent-toolchain 驱动为 Codex 项目接入' ;;
    opencode) configure_opencode_project ;;
  esac
}
...
config_has_codegraph_block() {
  config=$1
  [ -f "$config" ] || return 1
  block=$(opencode_mcp_block_json)
  python3 -c 'import json,sys
p, block = sys.argv[1], json.loads(sys.argv[2])
cfg = json.load(open(p))
expected = block["mcp"]["codegraph"]
current = cfg.get("mcp", {}).get("codegraph")
sys.exit(0 if current == expected else 1)' "$config" "$block" 2>/dev/null
}
...
- 安装、配置修复、初始化或修复索引、健康检查、升级审查和回滚使用全局 `$agent-toolchain`；不得在日常开发中自行安装、升级、重配或维护工具链。'
  if config_has_codegraph_block "$config"; then
    : # 已存在受管 CodeGraph 块，无需写入
  elif [ -f "$config" ] && rg -q '"codegraph"' "$config"; then
    die "opencode.json 存在非受管的 codegraph 配置；请人工核对后移除或改为受管格式"
  elif [ -f "$config" ] && ! python3 -c 'import json,sys; json.load(open(sys.argv[1]))' "$config" 2>/dev/null; then
    die "opencode.json 不是合法 JSON；请先修复再接入"
  else
...
p, block = sys.argv[1], json.loads(sys.argv[2])
cfg = json.load(open(p))
cfg.setdefault("mcp", {})["codegraph"] = block["mcp"]["codegraph"]
...
open(p, "a").write("\n")' "$config" "$block"
  ...
  note "项目 CodeGraph 与 RTK 受管配置已就绪（opencode.json mcp.codegraph）"
}
...
main() {
  ...
  case "$ACTION" in
    ...
    configure)
      ...
      [ "$CONFIG_TARGET" = opencode ] && { command -v python3 >/dev/null 2>&1 || die '缺少 python3；--config opencode 需要 python3 合并 JSON'; }
      ;;
    rollback)
      detect_platform
      load_trusted_manifest
