...
function Show-Usage {
  @'
Usage:
  agent-toolchain.ps1 configure --project PATH [--config codex|opencode]
...
function Parse-Arguments {
  ...
  while ($index -lt $DriverArgs.Count) {
    ...
    switch ($argument) {
      ...
      '--config' {
        if ($index + 1 -ge $DriverArgs.Count) { Fail '--config 缺少取值' }
        $script:Client = $DriverArgs[$index + 1]
        if ($script:Client -notin @('codex', 'opencode')) { Fail "--config 取值无效：$($script:Client)（可用：codex|opencode）" }
        $index += 2
      }
  ...
  if ($script:Client -and $script:Action -ne 'configure') { Fail '--config 仅适用于 configure' }
}
...
function Configure-Project {
  $opencodeConfigPath = Join-Path $script:Project 'opencode.json'
  $agentsPath = Join-Path $script:Project 'AGENTS.md'
  $ignorePath = Join-Path $script:Project '.gitignore'

  if ($script:Client -eq 'codex') { Fail 'opencode 适配包不再支持 --config codex 落点；请使用 shared 的 agent-toolchain 驱动为 Codex 项目接入' }

  Assert-PlainFileOrAbsent $opencodeConfigPath
  Assert-PlainFileOrAbsent $agentsPath
  Assert-PlainFileOrAbsent $ignorePath

  $expectedBlock = [ordered]@{
    type = 'local'
    command = @('codegraph', 'serve', '--mcp')
    enabled = $true
    environment = [ordered]@{ CODEGRAPH_TELEMETRY = '0'; CODEGRAPH_NO_UPDATE_CHECK = '1'; DO_NOT_TRACK = '1' }
  }

  $json = $null
  if (Test-Path -LiteralPath $opencodeConfigPath) {
    try { $json = Get-Content -LiteralPath $opencodeConfigPath -Raw | ConvertFrom-Json -AsHashtable } catch { Fail 'opencode.json 不是有效的 JSON' }
  } else {
    $json = [ordered]@{}
  }
  if ($json -isnot [System.Collections.IDictionary]) { Fail 'opencode.json 顶层必须是 JSON 对象' }
  if (-not $json.Contains('mcp')) { $json['mcp'] = [ordered]@{} }
  elseif ($json['mcp'] -isnot [System.Collections.IDictionary]) { Fail 'opencode.json 的 mcp 必须是对象' }
  $mcp = $json['mcp']
  $needsOpenCodeConfig = $false; $needsAgents = $false; $needsIgnore = $false
  if ($mcp.Contains('codegraph')) {
    $existing = $mcp['codegraph']
    $existingEnvironment = $null
    if ($existing -is [System.Collections.IDictionary]) { $existingEnvironment = $existing['environment'] }
    $matches = $existing -is [System.Collections.IDictionary] -and $existing['type'] -eq 'local' -and (@($existing['command']) -join ',') -eq 'codegraph,serve,--mcp' -and $existing['enabled'] -eq $true -and $existingEnvironment -is [System.Collections.IDictionary] -and $existingEnvironment['CODEGRAPH_TELEMETRY'] -eq '0' -and $existingEnvironment['CODEGRAPH_NO_UPDATE_CHECK'] -eq '1' -and $existingEnvironment['DO_NOT_TRACK'] -eq '1'
    if (-not $matches) { Fail 'opencode.json 的 CodeGraph 配置冲突' }
  } else {
    $mcp['codegraph'] = $expectedBlock
    $needsOpenCodeConfig = $true
  }
  ...
  if ($needsOpenCodeConfig) {
    [IO.File]::WriteAllText($opencodeConfigPath, "$($json | ConvertTo-Json -Depth 10)`n", [Text.UTF8Encoding]::new($false))
  }
  if ($needsAgents) {
