...
description: "用于复杂开发、跨文件实现或修复、架构设计、Bug 或漏洞诊断、复审和验收；按证据、风险和执行边界选择 subagent 角色，可拆任务优先多 agent 并行。"
---

# 多模型工作流编排（DeepSeek Harness 适配版）

本 Skill 规定复杂任务的协作行为。dsh 原生负责 subagent 的创建、上下文传递、通信、等待、中断和汇总（`subagent` 工具及其 spawn/fork 后端）；主 agent 负责核验真实状态、diff、产物和验证结果。

## 适用范围

简单问答、只读查询和边界清晰的单文件低风险整理由主 agent 直接完成并验证。跨文件、未知根因、需要独立复审、共享状态或高回归风险的任务按以下阶段推进：

`Explore -> Plan -> Work -> Critique -> Promote`

这些是行为阶段，不是固定的运行时节点或工具顺序。

默认并行优先：任务可拆成两个以上边界清晰、互不依赖且产出明确的子任务时，主 agent 应使用多个 subagent 同时处理，不应只交给单个 agent 串行执行。无法安全拆分、拆分成本高于收益或当前工具受限时，才使用单 agent。

## 协作边界

- 委派前明确目标、范围、权限、风险与回滚、验收、验证、停止条件，以及执行和集成责任；未决选择不得交给执行者自行补全。
- 子 agent 只能修改分配范围，不得覆盖或回滚其他改动；共享文件、数据和外部状态按必要顺序处理。
- 子 agent 只向直接委派它的父任务汇报。不得根据任务路径、历史会话 ID 或上下文中的旧 ID 自行选择会话；无法确认父任务时，只在自身最终消息返回，不跨会话发送。跨会话通信必须由主 agent 明确授权。
...
主 agent 基于证据明确目标、范围、非目标、授权、验收、验证和停止条件。多个 subagent 写入时，只划分完成任务所需的最小范围，优先按功能、模块或文件形成非重叠区域；共享部分按必要顺序处理，不因局部冲突取消其他可安全并行的工作。未决选择、权限、依赖或环境问题不得交给 executor 自行补全。
...
保留原始 provider/宿主错误、已尝试操作、影响范围和缺失条件。子 agent 因权限不足、缺少授权、目标超出边界或责任冲突而拒绝时，结束该次委派，保留原始拒绝，不再等待该 agent 或原样重复派发。先补齐授权、收缩范围、消除责任冲突或选择匹配的角色；拒绝条件已经改变时可以重新派发，并说明前次拒绝。拒绝不等于角色/模型档不可用，不触发替代。不得把写入任务派给只读角色。主 agent 仅在授权、权限、范围和责任边界均已满足时直接继续，否则向用户说明阻塞。中断后重新盘点 subagent 状态、工作区 diff 和输出；无法证明旧执行者已停止时，不得接管其范围。风险、授权或范围改变时回到 Plan，不得静默扩大。